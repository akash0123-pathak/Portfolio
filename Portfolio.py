...
- Keep answers concise but complete — finish every sentence fully"""
    ...
    if sent and typed:
        # Get clean recent history (last 3 exchanges only)
        history = [m for m in st.session_state.chat_history[-6:] if m["content"].strip()]
        st.session_state.chat_history.append({"role": "user", "content": typed})
        with st.spinner("⚡ Thinking..."):
            reply, source = get_answer(typed, history)

...
"""
    ...
    BASE_SYSTEM = """You are an AI assistant on Akash Pathak's portfolio website.
Answer ONLY the question asked. Use ONLY the facts from the profile below.

STRICT RULES:
- Answer the EXACT question asked — nothing else
- NEVER generate new questions or ask the user anything
- NEVER continue answering beyond what was asked
- NEVER say "If you have any specific questions..." or similar
- NEVER trail off mid-sentence or mid-word
- Use ONLY facts from the profile — never invent details
- Be direct, specific, and complete in 3-6 sentences max
- For unrelated questions: say "I only have info about Akash's professional profile." """
    ...
    if sent and typed:
        st.session_state.pending_msg = typed
        st.rerun()

    # Process pending message AFTER UI renders (prevents empty bubble)
    if "pending_msg" in st.session_state and st.session_state.pending_msg:
        q = st.session_state.pop("pending_msg")
        # Only pass last 4 messages as history to avoid context confusion
        history = [m for m in st.session_state.chat_history[-4:] if m["content"].strip()]
        st.session_state.chat_history.append({"role": "user", "content": q})
        with st.spinner("⚡ Thinking..."):
            reply, source = get_answer(q, history)

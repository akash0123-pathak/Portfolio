...
- Keep answers concise but complete — finish every sentence fully"""
    ...
    if sent and typed:
        st.session_state._pending_q = typed
        st.session_state._pending_hist = [m for m in st.session_state.chat_history[-6:] if m["content"].strip()]
    ...
    if st.session_state.get("_pending_q"):
        ...
        hist = st.session_state.pop("_pending_hist", [])
        with st.spinner("⚡ Thinking..."):
            reply, source = get_answer(q, hist)

...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── Knowledge chunks for ML semantic retrieval ─────────────────────────────
    # Each chunk is a focused fact — TF-IDF picks the most relevant ones
    # and injects them into the prompt for accurate, grounded answers
    CHUNKS = [
        "Akash Pathak is an AI/ML Engineer at TCS Indore since May 2025. He designs end-to-end ML workflows, builds TensorFlow models, implements Docker containers, CI/CD pipelines on AWS, and monitors production models for data drift.",
        "Akash completed two internships: IBM SkillsBuild ML Intern (Sep-Nov 2022) where he built Random Forest classifiers, did feature engineering and cross-validation. DMRC Delhi Metro Data Science Intern (Jul-Sep 2022) where he did data cleaning, outlier detection, and built a Streamlit analytics dashboard.",
        "Akash's education: B.Tech Computer Science Engineering (Data Science) from Oriental Institute of Science and Technology Bhopal, 2020 to 2024, CGPA 8.32. Class XII from Maharishi Vidya Mandir Jabalpur 69.2%. Class X CGPA 9.4.",
        "Akash's technical skills include Python, SQL, TensorFlow, Keras, PyTorch, Scikit-learn, LangChain, LangGraph, RAG, Prompt Engineering, HuggingFace, OpenAI APIs, Docker, AWS, CI/CD, MLOps, Tableau, Streamlit, Pandas, NumPy.",
        "Akash built an Enterprise Document Intelligence RAG system using LangChain and FAISS/Chroma vector search to automate knowledge retrieval from PDF DOCX and TXT documents.",
        "Akash built a Lung Disease Detection model using ResNet50 fine-tuned with TensorFlow on chest X-ray images to classify Normal, Pneumonia, and COVID-19 cases with image augmentation.",
        "Akash built an Indian Crime Dashboard using Streamlit and Plotly visualizing 10 years of India crime data with state-wise filters trend charts and EDA insights.",
        "Akash built a Smart Streetlight AI system using OpenCV and ML for real-time anomaly detection of flickering outages and brightness deviations with automatic alerts.",
        "Akash built a Driver Drowsiness Detection system using dlib facial landmarks and Eye Aspect Ratio EAR to detect fatigue at 30fps and trigger audio visual alarms.",
        "Akash built a Mental Health Tracker app using Random Forest classifier on questionnaire responses to give personalized wellness risk assessment and recommendations.",
        "Akash's certifications: Machine Learning with Python from FreeCodeCamp, Applied Data Science with Python from IBM SkillsBuild, Python for Data Science and Machine Learning from Udemy, Data Science A-Z from Udemy, Data Analysis Using Python from IBM.",
        "Contact Akash: email akash.pathak0123@gmail.com, phone +91 7024426415, LinkedIn linkedin.com/in/akash-pathak-44a082212, GitHub github.com/akash0123-pathak. He is based in Indore India originally from Chhatapur.",
        "Akash is open to new job opportunities in Generative AI, MLOps, LLM Engineering, and AI Product roles, remote or hybrid.",
        "Akash works with GenAI and LLM technologies: LangChain, LangGraph, RAG pipelines, Prompt Engineering, HuggingFace Transformers, OpenAI APIs for building intelligent applications.",
        "Akash uses MLOps tools: Docker for containerization, AWS for cloud deployments, CI/CD pipelines for automation, model monitoring and drift detection for production reliability.",
    ]

    BASE_SYSTEM = """You are an AI assistant embedded in Akash Pathak's portfolio website.

STRICT RULES:
- Give DIRECT, COMPLETE answers immediately. NEVER ask follow-up questions like "would you like to know more?" or "what aspect interests you?"
- Be SPECIFIC: always include names, dates, technologies, numbers
- NEVER say "and more" or trail off — list everything from the context
- When asked about skills: list ALL skills with categories
- When asked about experience: give company + role + dates + key responsibilities
- When asked about projects: name ALL 6 projects with tech stacks
- When asked about education: give degree + college + CGPA + year
- Keep answers 4-6 sentences, packed with real facts
- Be confident and professional, like a personal spokesperson
- Only answer about Akash. For unrelated questions, redirect once politely"""

    # ── ML: TF-IDF semantic retrieval ─────────────────────────────────────────
    ...
    def retrieve_context(query, top_k=3):
        """Use TF-IDF cosine similarity to find the most relevant chunks."""
        try:
            from sklearn.metrics.pairwise import cosine_similarity
            vec, mat = build_retriever()
            q_vec = vec.transform([query])
            scores = cosine_similarity(q_vec, mat).flatten()
            top_idx = scores.argsort()[-top_k:][::-1]
            # Only include chunks with meaningful similarity
            relevant = [CHUNKS[i] for i in top_idx if scores[i] > 0.05]
            return "\n".join(relevant) if relevant else "\n".join(CHUNKS[:3])
        except Exception:
            return "\n".join(CHUNKS[:3])

    def build_prompt(query):
        """Inject ML-retrieved relevant context into system prompt."""
    ...
    if not _key_ok:
        st.error("⚠️ GROQ_API_KEY not found in secrets. Add it in Streamlit → Settings → Secrets")
    else:
        st.success("✅ Groq API key loaded — chatbot ready!")

    # Pre-load retriever silently
    try:
        build_retriever()
    except Exception:
        pass
    ...
    if sent and typed:
        ...
        with st.spinner("Thinking..."):
            reply, source = get_answer(typed, st.session_state.chat_history[:-1])

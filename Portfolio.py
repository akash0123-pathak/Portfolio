...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    KB = {
        "what does akash do at tcs": "Akash is an AI/ML Engineer at TCS designing end-to-end ML workflows — preprocessing, training, deployment & monitoring. He builds TensorFlow models, uses Docker/CI/CD for AWS, and monitors production models for drift.",
        "tcs role responsibilities work": "At TCS, Akash builds scalable TensorFlow models, intelligent automation systems, containerizes ML services with Docker, and implements CI/CD pipelines on AWS. He also monitors models for data drift.",
        "top skills best skills technical": "Akash's core skills: Python, TensorFlow, PyTorch, Scikit-learn, LangChain, LangGraph, RAG, HuggingFace, Docker, AWS, SQL, MLOps.",
        "education degree college university cgpa": "B.Tech in CS Engineering (Data Science) from Oriental Institute of Science & Technology, Bhopal — 2020 to 2024, CGPA 8.32.",
        "rag project document intelligence langchain": "Akash built a RAG-based Enterprise Document Intelligence System using LangChain and FAISS/Chroma vector search — automates knowledge retrieval from PDFs, DOCX, and TXT documents.",
        "lung disease detection deep learning xray": "ResNet-based TensorFlow model for chest X-ray classification — detects Normal, Pneumonia, and COVID-19 cases using image augmentation and batch normalization.",
        "crime dashboard streamlit india analytics": "Interactive Streamlit + Plotly dashboard analyzing 10+ years of Indian crime data — state-wise filters, trend charts, and EDA insights.",
        "streetlight computer vision opencv anomaly": "Smart Streetlight AI system using OpenCV and ML for real-time anomaly detection — detects flickering, outages, and brightness deviations with automatic alerts.",
        "drowsiness detection driver safety opencv": "Real-time driver drowsiness detection using dlib facial landmarks and Eye Aspect Ratio (EAR) — triggers audio/visual alarm, runs at 30fps on webcam.",
        "mental health tracker nlp streamlit wellness": "ML-based mental health screening app using Random Forest on questionnaire responses — provides personalized risk assessment and wellness recommendations.",
        "projects portfolio built": "Akash's projects: RAG Document Intelligence, Lung Disease Detection (ResNet), Indian Crime Dashboard, Smart Streetlight AI, Driver Drowsiness Detection, Mental Health Tracker.",
        "ibm internship skillsbuild machine learning": "IBM SkillsBuild ML intern (Sep–Nov 2022): built Random Forest classifiers, performed feature engineering and cross-validation for mental health prediction.",
        "dmrc delhi metro internship data science": "DMRC Data Science intern (Jul–Sep 2022): data cleaning, outlier detection, and built a Streamlit analytics dashboard for the operations team.",
        "certifications certificates courses": "Certifications: ML with Python (FreeCodeCamp), Applied Data Science with Python (IBM), Python for DS & ML (Udemy), Data Science A-Z (Udemy), Data Analysis with Python (IBM).",
        "contact email phone linkedin github": "Email: akash.pathak0123@gmail.com | Phone: +91 7024426415 | LinkedIn: linkedin.com/in/akash-pathak-44a082212 | GitHub: github.com/akash0123-pathak",
        "open to work opportunities job hire": "Yes! Akash is open to roles in Generative AI, MLOps, LLM Engineering, and AI Product development — remote or hybrid.",
        "generative ai llm langchain langraph huggingface": "Akash works with LangChain, LangGraph, RAG pipelines, Prompt Engineering, HuggingFace models, and OpenAI APIs for GenAI applications.",
        "mlops docker aws cicd deployment monitoring": "Akash uses Docker for containerization, AWS for cloud deployments, CI/CD pipelines for automation, and monitors production models for drift and degradation.",
        "python tensorflow pytorch keras scikit": "Akash is proficient in Python, TensorFlow, Keras, PyTorch, and Scikit-learn for building and deploying ML/DL models.",
        "location city india where from": "Akash is based in Indore, India (originally from Chhatapur). Currently working at TCS Indore.",
    }

    # ── ML-based intent matcher using TF-IDF cosine similarity ─────────────────
    ...
    def build_matcher():
        ...
        keys = list(KB.keys())
        vec = TfidfVectorizer(ngram_range=(1,2), analyzer='word').fit(keys)
        mat = vec.transform(keys)
        return vec, mat, keys, np.array

    def smart_answer(question):
        try:
            vec, mat, keys, np_arr = build_matcher()
            from sklearn.metrics.pairwise import cosine_similarity
            q_vec = vec.transform([question.lower()])
            scores = cosine_similarity(q_vec, mat).flatten()
            best_idx = scores.argmax()
            if scores[best_idx] > 0.12:
                return KB[keys[best_idx]]
        except Exception:
            pass
        # Fallback: TinyLlama
        try:
            pipe = load_model()
            CONTEXT = (
                "You are Akash Pathak's portfolio assistant. Answer only about Akash, be concise (2-3 sentences). "
                "Akash is AI/ML Engineer at TCS. Skills: Python,TensorFlow,PyTorch,LangChain,RAG,Docker,AWS. "
                "Projects: RAG system,Lung Detection,Crime Dashboard,Streetlight AI,Drowsiness Detection,Mental Health Tracker. "
                "Education: B.Tech CS Data Science CGPA 8.32. Contact: akash.pathak0123@gmail.com"
            )
            prompt = f"<|system|>\n{CONTEXT}\n<|user|>\n{question}\n<|assistant|>"
            out = pipe(prompt, max_new_tokens=80, do_sample=False,
                       repetition_penalty=1.2, pad_token_id=pipe.tokenizer.eos_token_id)
            reply = out[0]["generated_text"].split("<|assistant|>")[-1].strip()
            for stop in ["<|user|>","<|system|>","\n\n"]:
                reply = reply.split(stop)[0].strip()
            return reply or "I'm not sure. Contact Akash at akash.pathak0123@gmail.com"
        except Exception:
            return "I'm not sure. Please contact Akash at akash.pathak0123@gmail.com"
    ...
    if not st.session_state.model_loaded:
        ...
        <div class='card' style='text-align:center;padding:32px;'>
            ...
            <div style='font-weight:700;color:#e2e2f0;font-size:1rem;margin-bottom:6px;'>Akash's AI Assistant</div>
            <div style='color:#8888aa;font-size:0.8rem;margin-bottom:20px;'>Loads once · Free · No API key</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if not st.session_state.chat_history:
            st.markdown("<div class='chat-bot'>👋 Hi! Ask me anything about Akash — experience, skills, projects, or contact info.</div>", unsafe_allow_html=True)
        ...
        with st.form("chat_form", clear_on_submit=True):
            ...
            with c_in:
                typed = st.text_input("", placeholder="Ask about Akash...", label_visibility="collapsed")
            with c_btn:

        ...
        with st.expander(f"{icon}  {name}{badge}  —  {summary}", expanded=featured):
            ...
            <div style='padding:4px 0;'>
                ...
                <div style='margin-top:14px;'>
                    <a class='link-btn' href='https://github.com/akash0123-pride' target='_blank'>🐙 View on GitHub</a>
                </div>
            </div>
            """, unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("""
    <p style='color:#8888aa;font-size:0.88rem;margin-bottom:20px;'>
    Powered by <b style='color:#a78bfa;'>TinyLlama-1.1B-Chat</b> — free, open-source, auto-downloaded
    from Hugging Face. No API key needed!
    </p>
    """, unsafe_allow_html=True)

    AKASH_CONTEXT = """You are a helpful assistant for Akash Pathak's portfolio. Answer ONLY about Akash. Be concise (2-4 sentences).

FACTS:
- Name: Akash Pathak. Role: AI/ML Engineer at TCS, Indore (May 2025-present).
- Education: B.Tech CS (Data Science), OIST Bhopal, 2020-2024, CGPA 8.32.
- Email: akash.pathak0123@gmail.com. Phone: +91 7024426415.
- LinkedIn: linkedin.com/in/akash-pathak-44a082212. GitHub: github.com/akash0123-pride.
- TCS work: end-to-end ML pipelines, TensorFlow models, Docker, CI/CD, AWS, model drift monitoring, intelligent automation.
- IBM SkillsBuild intern (Sep-Nov 2022): Random Forest, feature engineering, cross-validation.
- DMRC intern (Jul-Sep 2022): data cleaning, outlier detection, Streamlit analytics app.
- Projects: RAG Document Intelligence (LangChain, vector search), Lung Disease Detection (ResNet+TensorFlow), Indian Crime Dashboard (Streamlit+Plotly), Smart Streetlight AI (OpenCV), Driver Drowsiness Detection, Mental Health Tracker.
- Skills: Python, SQL, TensorFlow, PyTorch, Scikit-learn, LangChain, LangGraph, RAG, HuggingFace, Docker, AWS, MLOps, Tableau.
- Certifications: ML with Python (FreeCodeCamp), Applied Data Science (IBM), Python for ML (Udemy), Data Science A-Z (Udemy), Data Analysis (IBM).
- Open to new opportunities in GenAI, MLOps, LLM Engineering."""

    QUICK_ANSWERS = {
        "what does akash do at tcs": "At TCS, Akash works as an AI/ML Engineer designing end-to-end ML workflows — preprocessing, model training, deployment & monitoring. He builds TensorFlow models, containerizes services with Docker, implements CI/CD for AWS, and monitors production models for drift.",
        "what are his top skills": "Akash's top skills are Python, TensorFlow, PyTorch, Scikit-learn, LangChain/LangGraph (RAG), Docker, and AWS. He's also actively learning MLOps.",
        "tell me about his rag project": "Akash built an Enterprise Document Intelligence System using LangChain and vector similarity search (FAISS/Chroma). It's a RAG pipeline for automated knowledge retrieval from large enterprise document collections — supporting PDF, DOCX, and TXT formats.",
        "what's his educational background": "Akash has a B.Tech in Computer Science Engineering (Data Science) from Oriental Institute of Science & Technology, Bhopal (2020–2024) with a CGPA of 8.32.",
        "is he open to new opportunities": "Yes! Akash is actively open to new opportunities, especially in Generative AI, MLOps, LLM Engineering, and AI Product roles.",
        "how can i contact akash": "You can reach Akash at akash.pathak0123@gmail.com or call +91 7024426415. Find him on LinkedIn (akash-pathak-44a082212) or GitHub (akash0123-pride).",
    }
    ...
    if not st.session_state.model_loaded:
        ...
        <div class='card' style='text-align:center;'>
            ...
            <div style='font-family:Space Mono,monospace;font-size:0.85rem;color:#a78bfa;margin-bottom:6px;'>TinyLlama-1.1B Chat Model</div>
            <div style='color:#8888aa;font-size:0.8rem;'>~600MB · Downloads once · Runs locally · 100% Free</div>
        </div>
        """, unsafe_allow_html=True)
        if st.button("🚀 Load AI Model", use_container_width=True):
            with st.spinner("⏳ Downloading TinyLlama from Hugging Face... (first time ~1-2 min)"):
                try:
                    load_model()
                    st.session_state.model_loaded = True
                    st.success("✅ Model loaded! Chat is ready.")
                    st.rerun()
        ...
        def get_answer(q):
            ...
            for k, v in QUICK_ANSWERS.items():
                if k in key or key in k:
                    return v
            prompt = f"<|system|>\n{AKASH_CONTEXT}\n<|user|>\n{q}\n<|assistant|>"
            out = pipe(prompt, max_new_tokens=120, do_sample=False,
                       repetition_penalty=1.3, pad_token_id=pipe.tokenizer.eos_token_id)
            reply = out[0]["generated_text"].split("<|assistant|>")[-1].strip()
            for stop in ["<|user|>","<|system|>","\n\n\n"]:
                reply = reply.split(stop)[0].strip()
            return reply or "Please check Akash's resume or contact him directly!"

        if "chat_history" not in st.session_state:
            st.session_state.chat_history = []

        if not st.session_state.chat_history:
            st.markdown("<div class='chat-bot'>👋 Hi! I'm Akash's AI. Ask me about his experience, skills, or projects!</div>", unsafe_allow_html=True)
        ...
        st.markdown("<div class='sub-label' style='margin:16px 0 8px;'>Quick Questions</div>", unsafe_allow_html=True)
        qqs = ["What does Akash do at TCS?","What are his top skills?","Tell me about his RAG project",
               "What's his educational background?","Is he open to new opportunities?","How can I contact Akash?"]
        for i in range(0, len(qqs), 2):
            ca, cb = st.columns(2)
            with ca:
                if st.button(qqs[i], key=f"q{i}", use_container_width=True):
                    st.session_state.pending_q = qqs[i]; st.rerun()
            with cb:
                if i+1 < len(qqs) and st.button(qqs[i+1], key=f"q{i+1}", use_container_width=True):
                    st.session_state.pending_q = qqs[i+1]; st.rerun()
        ...
        st.markdown("""
        <div style='margin-top:16px;padding:10px 14px;background:rgba(74,222,128,0.05);
             border:1px solid rgba(74,222,128,0.2);border-radius:10px;
             font-family:Space Mono,monospace;font-size:0.7rem;color:#4ade80;'>
        ✅ TinyLlama-1.1B-Chat · No API key · Auto-cached after first download
        </div>
        """, unsafe_allow_html=True)

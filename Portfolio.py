...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    BASE_SYSTEM = """You are an AI assistant on Akash Pathak's portfolio. Answer only about Akash — professionally and conversationally. Keep answers to 3-5 sentences. If asked something unrelated, politely redirect to Akash's profile."""
    ...
    def ask_groq(query, history):
        try:
            ...
            return None, "groq_error"
        except Exception:
            return None, "groq_fail"

    # ── Layer 2: TinyLlama backup ──────────────────────────────────────────────
    @st.cache_resource(show_spinner=False)
    def load_tinyllama():
    ...
    def ask_tinyllama(query):
        try:
            ...
            return reply or None, "tinyllama"
    ...
    def get_answer(query, history):
        ...
        reply, source = ask_tinyllama(query)
        if reply:
            return reply, "🔁 TinyLlama"
        return "I couldn't process that. Contact Akash at akash.pathak0123@gmail.com", "❌"
    ...
    with st.form("chat_form", clear_on_submit=True):
        ...
        with c_in:
            typed = st.text_input("", placeholder="Ask anything about Akash...", label_visibility="collapsed")
        with c_btn:
    ...
    # Groq setup notice only if no key set
    groq_key = st.secrets.get("GROQ_API_KEY", "")
    if not groq_key:
        st.markdown("""
        <div style='margin-top:16px;padding:12px 16px;background:rgba(251,191,36,0.07);
             border:1px solid rgba(251,191,36,0.25);border-radius:10px;
             font-family:Space Mono,monospace;font-size:0.7rem;color:#fbbf24;'>
        ⚙️ <b>For best results (Llama 3):</b> Get a free key at <b>console.groq.com</b>
        → Streamlit Settings → Secrets → add <code>GROQ_API_KEY = "gsk_..."</code><br>
        Without it, TinyLlama is used as fallback.
        </div>
        """, unsafe_allow_html=True)
